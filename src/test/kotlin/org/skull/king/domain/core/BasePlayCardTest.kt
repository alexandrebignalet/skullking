...
import org.skull.king.core.usecases.captor.Play
...
class BasePlayCardTest {
    ...
    fun `Should return an error if game not started`() {
        ...
        val playCard = PlayCardSaga(gameId, playerId, ClassicConfiguration().deck().cards.first())
    ...
    inner class OnCardPlayed {
        ...
        fun `Should store card played when all players announced`() {
            ...
            val cardPlayed = currentPlayer.cards.first()
        ...
        fun `Should return an error when card played and player do not have the card chosen`() {
            ...
            val cardPlayed = otherPlayer.cards.first()
            val playCard = PlayCardSaga(gameId, currentPlayer.id, cardPlayed)
        ...
        fun `Should remove card played from player hand and add it to game fold`() {
            ...
            await atMost Duration.ofSeconds(1) untilAsserted {
                ...
                Assertions.assertThat(play?.card).isEqualTo(ReadCard.from(playedCard))
            }
        }
        ...
        fun `Should resolve the fold winner when last player played his card`() {
            ...
            await atMost Duration.of(5, ChronoUnit.SECONDS) untilAsserted {
                ...
                Assertions.assertThat(game.scoreBoard[firstPlayer.id]?.find { it.roundNb == roundNb }?.done)
                    .isEqualTo(1)
                Assertions.assertThat(game.scoreBoard[firstPlayer.id]?.find { it.roundNb == roundNb }?.potentialBonus)
                    .isEqualTo(50)

                Assertions.assertThat(game.scoreBoard[secondPlayer.id]?.find { it.roundNb == roundNb }?.announced)
                    .isEqualTo(futureLoserAnnounce)
                Assertions.assertThat(game.scoreBoard[secondPlayer.id]?.find { it.roundNb == roundNb }?.done)
                    .isEqualTo(0)
                Assertions.assertThat(game.scoreBoard[secondPlayer.id]?.find { it.roundNb == roundNb }?.potentialBonus)
                    .isEqualTo(0)
            }
        }
    }
